...
CONFIG_DIR="/g/kreshuk/talks/consistency_results/Instance_segmentation/Cells/configs/Instance_seg_04th"
...
# Loop through all seg_config_*.yaml files in the config directory
for config_file in "$CONFIG_DIR"/*seg_config*.yaml; do
    # Check if file exists (in case no files match the pattern)
    if [[ ! -f "$config_file" ]]; then
        echo "No config files found matching pattern *seg_config*.yaml"
        continue
    fi
...
#SBATCH --mem 5G			                    # specify the amount of memory per node
...
python segmentation_from_boundaries.py --config $config_file
EOF
