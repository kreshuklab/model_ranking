...
CONFIG_DIR="/g/kreshuk/talks/consistency_results/patch_segmentation/mitochondria/ccfv_scores/configs/R_Target"
...
for config_file in "$CONFIG_DIR"/*meta_config*.yaml; do
    ...
    if [[ ! -f "$config_file" ]]; then
        echo "No config files found matching pattern meta_config_*.yaml"
        continue
    fi
...
#SBATCH --mem 100G			                    # specify the amount of memory per node
...
python run_CCFV.py --config $config_file
EOF
