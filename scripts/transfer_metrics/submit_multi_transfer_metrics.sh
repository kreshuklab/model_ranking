...
CONFIG_DIR="/g/kreshuk/talks/sampled_features/semantic_segmentation/mitochondria/1k_pixels_sampled/transfer_metric_results/configs/AdaBN_redo"
...
# Loop through all self_training_config_*.yaml files in the config directory
for config_file in "$CONFIG_DIR"/*metric_config*.yaml; do
    # Check if file exists (in case no files match the pattern)
    if [[ ! -f "$config_file" ]]; then
        echo "No config files found matching pattern metric_config*.yaml"
        continue
    fi
    ...
    # Extract the substring before "metric_config"
    substring="${filename%%metric_config*}"
...
python calculate_transferability_metric.py --config $config_file
EOF
