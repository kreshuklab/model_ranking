...
CONFIG_DIR="/g/kreshuk/talks/sampled_features/semantic_segmentation/mitochondria/1k_pixels_sampled/configs/AdaBN_features/U-Net"
...
for config_file in "$CONFIG_DIR"/*config*.yaml; do
    ...
    if [[ ! -f "$config_file" ]]; then
        echo "No config files found matching pattern self_training_config_*.yaml"
        continue
    fi
    ...
    # Extract the substring before "_config"
    substring="${filename%_config*.yaml}"
...
python extract_transfer_features.py --config $config_file
EOF
