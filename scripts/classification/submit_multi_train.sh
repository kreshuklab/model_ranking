...
CONFIG_DIR="/g/kreshuk/talks/classification_models/Mitochondria/configs/RN50"
...
# Loop through all config.yaml files in the config directory
for config_file in "$CONFIG_DIR"/*config.yaml; do
    # Check if file exists (in case no files match the pattern)
    if [[ ! -f "$config_file" ]]; then
        echo "No config files found matching pattern *config.yaml"
        continue
    fi
...
python train_classification_model.py --config $config_file
EOF
